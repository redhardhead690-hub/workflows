...
def build_prompt(by_journal: dict):
    """Turn the collected articles into a prompt. Returns (prompt, has_content)."""
    header = (
        ...
        "organizational consulting and leadership development. For each "
        "journal that has new articles, write one sentence of overview, then "
        "for each article give: a 1-2 sentence plain-language summary of the "
        "finding in your own words (do not copy the abstract), followed by a "
        "short italic 'Why it matters for practice' note. Skip journals with "
        "no new articles. Keep it skimmable. Finish with a single-line "
        "'Theme of the period' if you notice a common thread.\n\n"
